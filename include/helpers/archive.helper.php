...
if ( ! function_exists('GetOneArchive'))
{
    ...
    {
        global $dsql;
        include_once(SUNINC."/channelunit.func.php");
        ...
        if(!isset($arcRow['descr'])) {
            $arcRow['descr'] = '';
        }

        if(empty($arcRow['descr']) && isset($arcRow['gbody'])) {
            $arcRow['descr'] = cn_substr(html2text($arcRow['gbody']), 250);
        }
