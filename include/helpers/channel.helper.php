<?php  if(!defined('SUNINC')) exit('suncms');
/**
 * 文档小助手
 *
 * @version        $Id: channel.helper.php 1 16:49 2010年7月6日Z tianya $
 * @package        SunCMS.Helpers
...
{
    function GetFileUrl($aid,$typeid,$timetag,$title,$namerule='',$typedir='',$filename='',$siteurl='')
    {
        $articleUrl = GetFileName($aid,$typeid,$timetag,$title,$namerule,$typedir,$filename);
...
{
     function GetFileNewName($aid,$typeid,$timetag,$title,$namerule='',$typedir='',$filename='')
    {
        global $cfg_arc_dirname;
        $articlename = GetFileName($aid,$typeid,$timetag,$title,$namerule,$typedir,$filename);
...
{
    function GetFileName($aid,$typeid,$timetag,$title,$namerule='',$typedir='',$filename='')
    {
        ...
        if(empty($namerule)) {
            ...
            //$typeid = -1;
			$namerule ="{typedir}/{aid}-{Y}{M}{D}.html";
        }
        if($typeid==0 )
        {
...
{
    function GetTypeUrl($typeid,$typedir,$isdefault,$defaultname,$namerule2,$siteurl='')
    {
...
function GetSonIds($id,$addthis=true)
{
    global $cfg_Cs;
...
function MakeOneTag(&$dtp, &$refObj, $parfield='Y')
{
    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        if(in_array($tagname,$alltags))
        {
            ...
            $filename = SUNINC.'/taglib/'.$tagname.'.lib.php';
...
function GetOneTypeUrlA($typeinfos)
{
    ...
    $typeinfos['namerule2'],$typeinfos['siteurl']);
}
...
function GetBookUrl($bid,$title,$gdir=0)
{
    global $cfg_cmspath;
    $bookurl = $gdir==1 ?
    "{$cfg_cmspath}/book/".sunID2Dir($bid) : "{$cfg_cmspath}/book/".sunID2Dir($bid).'/'.GetPinyin($title).'-'.$bid.'.html';
    return $bookurl;
}
