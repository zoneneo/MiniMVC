...
if ( ! function_exists('WaterImg'))
{
    ...
    {
        include(SUNDATA.'/mark/inc_photowatermark_config.php');
        require_once(SUNINC.'/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = SUNDATA.'/mark/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(SUNDATA.'/mark/simhei.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  SUNDATA.'/mark/simhei.ttf';
            }
            else
            {
                return ;
