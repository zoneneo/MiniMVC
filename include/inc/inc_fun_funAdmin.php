...
function SpGetPinyin($str, $ishead=0, $isclose=1)
{
    ...
    if(count($pinyins) == 0)
    {
        $fp = fopen(SUNINC.'/data/pinyin.dat', 'r');
        while(!feof($fp))
        {
            $line = trim(fgets($fp));
...
function SpGetEditor($fname,$fvalue,$nheight="350",$etype="Basic",$gtype="print",$isfullpage="false",$bbcode=false)
{
    ...
    {
        require_once(SUNINC.'/FCKeditor/fckeditor.php');
    ...
    {
        require_once(SUNINC.'/ckeditor/ckeditor.php');
        $CKEditor = new CKEditor();
        $CKEditor->basePath = $GLOBALS['cfg_cmspath'].'/include/ckeditor/' ;
        $config = $events = array();
        $config['extraPlugins'] = 'sunpage,multipic,addon';
		if($bbcode)
		{
			...
			// 获取表情信息
			require_once(SUNDATA.'/smiley.data.php');
			$jsscript = array();
        ...
        /*
        // ------------------------------------------------------------------------
        // 当前版本,暂时取消sunhtml编辑器的支持
        // ------------------------------------------------------------------------
        require_once(SUNINC.'/htmledit/sun_editor.php');
        $ded = new sunEditor($fname);
