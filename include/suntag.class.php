 ...
 * Sun织梦模板类
 *
 * @version        $Id: suntag.class.php 1 10:33 2010年7月6日Z tianya $
 * @package        SunCMS.Libraries
 * @copyright      Copyright (c) 2016 - , Inc.
 */
/**
 * class SunTag 标记的数据结构描述
 * function c____SunTag();
 *
 * @package          SunTag
...
class SunTag
{
    ...
    var $EndPos=0;        //标记结束位置
    var $CAttribute="";   //标记属性描述,即是class SunAttribute
    var $TagValue="";     //标记的值
    var $TagID = 0;
...
/**
 * SunTagParse Sun织梦模板类
 * function c____SunTagParse();
 *
 * @package          SunTagParse
 * @subpackage       SunCMS.Libraries
...
class SunTagParse
{
        ...
        if (defined('SUNDISFUN')) {
            ...
            foreach ($tokens as $token)
            {
                if (is_array($token))
                {
                    ...
                    {
                       $errmsg = 'SunCMS Error:function disabled "'.$token[1].'" <a href="http://help.suncms.com/install-use/apply/2013/0711/2324.html" target="_blank">more...</a>';
                       return FALSE;
                    }
                }
            }
    ...
    function LoadCache($filename)
    {
        ...
        if( isset($z) && is_array($z) )
        {
            foreach($z as $k=>$v)
            {
                $this->Count++;
                $ctag = new SunTAg();
                $ctag->CAttribute = new SunAttribute();
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        {
            $SunMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$SunMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $SunMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
        ...
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                {
                    //if '/}' more near '{sun:'、'{/sun:' , end tag is '/}', else is '{/sun:'
                    if($e1 < $e2 &&  $e1 < $e3 )
                    {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
    ...
    {
        $SunFieldValue = $fieldvalue;
        $functionname = str_replace("{\"","[\"",$functionname);
        $functionname = str_replace("\"}","\"]",$functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i",'$SunFieldValue',$functionname);
        $functionname = "\$SunFieldValue = ".$functionname;
        @eval($functionname.";"); //or die("<xmp>$functionname</xmp>");
        if(empty($SunFieldValue))
        {
            return '';
        }
        else
        {
            return $SunFieldValue;
        }
    }
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(SUNROOT."/templets/".$filename) )
        {
            $okfile = SUNROOT."/templets/".$filename;
        }
        else if(file_exists(SUNROOT.'/templets/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = SUNROOT.'/templets/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
...
/**********************************************
//class SunAttribute Sun模板标记属性集合
function c____SunAttribute();
**********************************************/
//属性的数据描述
class SunAttribute
{
...
function c____SunAttributeParse();
********************************/
class SunAttributeParse
{
    var $sourceString = "";
    ...
    {
        $this->cAttributes = new SunAttribute();
        $strLen = 0;
