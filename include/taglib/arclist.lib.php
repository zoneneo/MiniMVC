...
<demo>
{sun:arclist  flag='h' typeid='' row='' col='' titlelen='' infolen='' imgwidth='' imgheight='' listtype='' orderby='' keyword='' limit='0,1'}
        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
    ...
    if($idlist=='')
    {
        ...
        if($keyword!='')
        {
            ...
            $orwheres[] = " CONCAT(arc.title,arc.words) REGEXP '$keyword' ";
        }
        //文档属性
...
/*     if($orderby=='hot' || $orderby=='click') $ordersql = " ORDER BY arc.click $orderWay";
    ...
    $query = "SELECT arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,tp.namerule2
    ...
    for($i=0; $i<$line; $i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
                $ids[] = $row['id'];
                //处理一些特殊字段
                $row['info'] = $row['infos'] = cn_substr($row['descr'],$infolen);
                $row['id'] =  $row['id'];
                ...
                $row['filename'] = $row['arcurl'] = GetFileUrl($row['id'],$row['typeid'],$row['senddate'],$row['title'],
				$row['namerule'],$row['typedir'],$row['filename'],$row['siteurl']);
...
function lib_GetAutoChannelID($sortid, $topid)
{
    ...
    $getstart = $sortid - 1;
    $row = $dsql->GetOne("SELECT id,typename FROM #@__arctype WHERE reid='{$topid}' ORDER BY sortrank asc limit $getstart,1");
