...
function lib_flink(&$ctag,&$refObj)
{
    ...
    else if($typeid == 999)
	{
		...
		if(!is_array($row))
		{
			require (SUNDATA.'/admin/config_update.php');
			if (!class_exists('sunHttpDown', false)) {
				require_once(SUNINC.'/sunhttpdown.class.php');
			}
			$del = new sunHttpDown();
			...
			if(!empty($linkInfo)){
				$sunlink = explode("\t", $linkInfo);
				for($i=0; $i<count($sunlink); $i++) {
					if($i%5==0 && $i!=count($sunlink)) {
						$revalue .= "<li><a href='http://".@$sunlink[$i+1]."' target='_blank' title='".@$sunlink[$i+4]."'>".@$sunlink[$i]."</a></li>";
					}
				}
			} else {
				$revalue=<<<EOT
<li><a href='http://ad.suncms.com' target='_blank' title='SunCMS广告'>SunCMS广告</a></li><li><a href='http://service.suncms.com' target='_blank' title='织梦客户服务中心'>
织梦客户服务中心</a></li><li><a href='http://ask.suncms.com' target='_blank' title='织梦问答'>
织梦问答</a></li><li><a href='http://tools.suncms.com' target='_blank' title='站长工具'>
站长工具</a></li><li><a href='http://site.desdev.cn' target='_blank' title='SunCMS建站中心'>
SunCMS建站中心</a></li><li><a href='http://help.suncms.com' target='_blank' title='织梦CMS帮助中心'>
