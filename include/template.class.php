...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
    ...
    $plusfile = SUNINC.'/tpllib/plus_'.$atts['tagname'].'.php';
...
class SunTemplate
{
    ...
    var $refObj = '';
    var $makeLoop = 0;
    var $tagStartWord =  '{sun:';
    var $fullTagEndWord =  '{/sun:';
    var $sTagEndWord = '/}';
    var $tagEndWord = '}';
    var $tpCfgs = array();
    ...
    //构造函数,兼容PHP4
    function SunTemplate($templatedir='',$refDir='')
    {
     ...
     * @return    void
     */
    function SetTagStyle($ts='{sun:',$ftend='{/sun:',$stend='/}',$tend='}')
    {
        ...
        if (!defined('SUNDISFUN')) {
            ...
            foreach ($tokens as $token)
            {
                if (is_array($token))
                {
                    ...
                    {
                       $errmsg = 'SunCMS Error:function disabled "'.$token[1].'"';
                       return FALSE;
                    }
                }
            }
    ...
    function CompilerInclude($filename, $isload=TRUE)
    {
        ...
        if( !$isload ) return 1;
		echo $okfile;
        $itpl = new SunTemplate($this->templateDir);
