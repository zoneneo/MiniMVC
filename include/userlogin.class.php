...
{
    $cacheFile = SUNATA.'/cache/addon-'.session_id().'.inc';
...
class userLogin
{
    ...
    var $userChannel = '';
    var $userPurview = '';
    var $keepUserIDTag = 'sun_admin_id';
    var $keepUserTypeTag = 'sun_admin_type';
    var $keepUserChannelTag = 'sun_admin_channel';
    var $keepUserNameTag = 'sun_admin_name';
    var $keepUserPurviewTag = 'sun_admin_purview';
    var $keepAdminStyleTag = 'sun_admin_style';
    var $adminStyle = 'suncms';
    ...
    function keepUser()
    {
        if($this->userID != '' && $this->userType != '')
        {
            ...
            if(empty($adminstyle)) $adminstyle = 'suncms';
            ...
            PutCookie('SunUserID', $this->userID, 3600 * 24, '/');
            PutCookie('SunLoginTime', time(), 3600 * 24, '/');
    ...
    {
        //$this->userChannel
        $cacheFile = SUNATA.'/cache/admincat_'.$this->userID.'.inc';
        //管理员管理的频道列表
    ...
    function exitUser()
    {
        ...
        DropCookie('sunAdmindir');
        DropCookie('sunUserID');
        DropCookie('sunLoginTime');
        $_SESSION = array();
    }
...
function GetSonIdsUL($id, $channel=0, $addthis=TRUE)
{
    ...
    if( !is_array($cfg_Cs) )
    {
        require_once(SUNATA."/cache/inc_catalog_base.inc");
    }
