...
define('SUNADM', str_replace("\\", '/', dirname(__FILE__) ) );
define('SUNROOT', substr(SUNADM,0,strrpos(SUNADM,'/')));
define('SUNINC',SUNROOT.'/include');
#define('SUNROOT',preg_replace("#[\\\\\/]install#", '', dirname(__FILE__)));
...
else if($step==2)
{
    ...
    $fp = fopen(SUNADM."/common.inc.php","r");
    ...
    @chmod(SUNINC,0775);
    $fp = fopen(SUNINC."/db.inc.php","w") or die("<script>alert('写入配置失败，请检查目录是否可写入！');history.go(-1);</script>");
