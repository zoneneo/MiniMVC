...
if($action=='')
{
    require_once(SUNADMIN."/templets/makehtml_all.htm");
    exit();
}
else if($action=='make')
{
    ...
    if($step==3)
    {
        include_once(SUNINC."/arc.partview.class.php");
        $pv = new PartView();
        $row = $pv->dsql->GetOne("SELECT * FROM `#@__homepageset` ");
		$templet = str_replace("{style}", $cfg_df_style,$row['templet']);
		$homeFile = SUNADMIN.'/'.$row['position'];
    ...
    else if($step==4)
    {
        ...
        $mkcachefile = SUNDATA."/mkall_cache_{$adminID}.php";
