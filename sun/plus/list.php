...
{	
    ...
    include(SUNINC."/arc.sglistview.class.php");
    $lv = new SgListView($tid,$cArr);
} else {	
    include(SUNINC."/arc.listview.class.php");
    $lv = new ListView($tid);
    //对设置了会员级别的栏目进行处理
    if(isset($lv->Fields['corank']) && $lv->Fields['corank'] > 0)
    {	
        /*
		require_once(SUNINC.'/memberlogin.class.php');
        ...
        if( $cfg_ml->M_Rank < $lv->Fields['corank'] )
        {
            ...
            include_once(SUNTEMPLATE.'/plus/view_msg_catalog.htm');
            exit();
        }
		*/
    }
