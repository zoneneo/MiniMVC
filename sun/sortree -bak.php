<?php
	...
	foreach($result as $k)
	{
		...
		if(is_numeric($k))
		{
			$cont .="<h3><b>$k . <a href='index.php?ct=admin&ac=table&to=archives&tid=$k'> ".$res[$k]." </a></b>";
		...
		}else
		{
			...
			{
				$k=str_replace('{','',$k);
				$cont .="<h3><b onclick='ShowNode($k)'>$k [<span id='e$k'>-</span>]<a href='index.php?ct=admin&ac=table&to=archives&tid=$k' > ".$res[$k]."</a></b>";
				$cont .=" <span><a href='javascript:JSMgSort(1,$k);'>子类</a> <a href='javascript:JSMgSort(2,$k);'>类名</a>";
